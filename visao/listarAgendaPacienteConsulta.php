<div class="container">
	...
	<table class="table table-striped">
		<caption><h4>Agenda Paciente - Consultas</h4></caption>
		<?php if(count($oAgendaPacienteConsultas) == 0){
			?> <div class="alert alert-info" role="alert">Nenhuma consulta agendada para este médico</div> <?php
		} ?>
		<thead>
			<tr>
				<th>N&ordm;</th>
		...
		<?php
			foreach($oAgendaPacienteConsultas as $oAgendaPaciente){
				?><tr>
					<td><?php echo $oAgendaPaciente->getId()."<br>";?></td>
						...
						<form method="post" action="?area=agendaPacienteConsultaControle">
							<input type="hidden" name="idAgendaPacienteConsulta" value="<?php echo $oAgendaPaciente->getId(); ?>" />
  ...
  <ul class="pagination background-color">
	<?php for($cont = 0; $cont < $_SESSION['numero_pag']; ++$cont){
		...
		?><li><a href="?area=agendaPacienteConsultaControle&acao=btListar&pagina=<?php echo $cont ?>"><?php echo $cont+1 ?></a></li> <?php
	} ?>
  </ul>
</nav>
	</div>
