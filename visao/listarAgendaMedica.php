<div class="container">
	...
	<table class="table table-striped">
		...
		<?php
			foreach($oAgendasMedica as $oAgendaMedica){
				?><tr>
					<td><?php echo $oAgendaMedica->getAgenda_id()."<br>";?></td>
                                        <td><?php if($oAgendaMedica->getOAgenda()->getDia_semana() == 1)
                                                    echo "Segunda-feira<br>";
                                                  else if($oAgendaMedica->getOAgenda()->getDia_semana() == 2)
                                                    echo "Terça-feira<br>";
                                                  else if($oAgendaMedica->getOAgenda()->getDia_semana() == 3)
                                                    echo "Quarta-feira<br>";
                                                  else if($oAgendaMedica->getOAgenda()->getDia_semana() == 4)
                                                    echo "Quinta-feira<br>";
                                                  else if($oAgendaMedica->getOAgenda()->getDia_semana() == 5)
                                                    echo "Sexta-feira<br>";
                                                  else if($oAgendaMedica->getOAgenda()->getDia_semana() == 6)
                                                    echo "Sábado<br>";
                                                  
                                        ?></td>
                                        <td><?php echo $oAgendaMedica->getOMedico()->getOPessoa()->getNome()."<br>";?></td>
                                        <td><?php echo $oAgendaMedica->getOMedico()->getOPessoa()->getTelefone()."<br>";?></td>
                                        <td><?php echo $oAgendaMedica->getOAgenda()->getOLocal()->getNome()."<br>"; ?></td>
                                        <td class="text-right col-md-1">
                                            <?php if($oAgendaMedica->getOAgenda()->getAtivo() == 1){ ?>
                                                <form method="post" action="?area=agendaMedicaControle">
							<input type="hidden" name="idAgendaMedica" value="<?php echo $oAgendaMedica->getAgenda_id(); ?>" />
  ...
  <ul class="pagination background-color">
	<?php for($cont = 0; $cont < $_SESSION['numero_pag']; ++$cont){
		...
		?><li><a href="?area=agendaMedicaControle&acao=btBuscar&busca=&pagina=<?php echo $cont ?>"><?php echo $cont+1 ?></a></li> <?php
	} ?>
  </ul>
</nav>
	</div>
